const { AppError } = require('../utils/appError');

exports.createFuelType = async (fuelTypeData) => {
    const { user_id, fuel_type_name, status } = fuelTypeData;

    // Check if the user exists
    const user = await User.findByPk(user_id);
    if (!user) {
        throw new AppError('User not found.', 404);
    }

    const fuelType = await FuelType.findOne({ where: { fuel_type_name: fuel_type_name.trim().toLowerCase() } });
    if (fuelType) {
        throw new AppError('Fuel type already exists.', 400);
    }

    const newFuelType = await FuelType.create({ user_id, fuel_type_name: fuel_type_name.trim().toLowerCase(), status });

    return newFuelType;
}

exports.getFuelTypes = async () => {
    const fuelTypes = await FuelType.findAll({
        include: [{
            model: User,
            attributes: ['id', 'full_name'] // Adjust attributes as needed
        }]
    });

    return fuelTypes;
}

exports.getFuelType = async (fuel_type_id) => {
    const fuelType = await FuelType.findByPk(fuel_type_id, {
        include: [{
            model: User,
            attributes: ['id', 'full_name'] // Adjust attributes as needed
        }]
    });

    return fuelType;
}

exports.updateFuelType = async (fuel_type_id, fuelTypeData) => {
    const { user_id, fuel_type_name, status } = fuelTypeData;

    const fuelType = await FuelType.findByPk(fuel_type_id);
    if (!fuelType) {
        throw new AppError('Fuel type not found.', 404);
    }

    // Check if the user exists
    const user = await User.findByPk(user_id);
    if (!user) {
        throw new AppError('User not found.', 404);
    }

    const updatedFuelType = await FuelType.update({ user_id, fuel_type_name: fuel_type_name.trim().toLowerCase(), status }, { where: { fuel_type_id } });

    return updatedFuelType;
}

exports.deleteFuelType = async (fuel_type_id) => {

    const fuelType = await FuelType.findByPk(fuel_type_id);
    if (!fuelType) {
        throw new AppError('Fuel type not found.', 404);
    }

    const deletedFuelType = await FuelType.destroy({ where: { fuel_type_id } });

    return deletedFuelType;
}

exports.updateFuelTypeStatus = async (fuel_type_id, status) => {
    const fuelType = await FuelType.findByPk(fuel_type_id);
    if (!fuelType) {
        throw new AppError('Fuel type not found.', 404);
    }

    const updatedFuelType = await FuelType.update({ status }, { where: { fuel_type_id } });

    return updatedFuelType;
}

exports.getFuelTypesByStatus = async (status) => {
    const fuelTypes = await FuelType.findAll({
        where: { status },
        include: [{
            model: User,
            attributes: ['id', 'full_name']
        }]
    });

    return fuelTypes;
}

