...
const carQuerySchema = Joi.object({
  ...
  min_year: Joi.number().integer().min(1900).max(2100).optional(),
  max_year: Joi.number().integer().min(1900).max(2100).optional(),
  ...
  user_id: Joi.alternatives().try(optionalHybridId, Joi.string().valid('me')).optional(),
  seller_id: Joi.alternatives().try(optionalHybridId, Joi.string().valid('me')).optional(),
  posted_by_me: Joi.alternatives().try(Joi.boolean(), Joi.string().valid('true', 'false')).optional(),
  mine: Joi.alternatives().try(Joi.boolean(), Joi.string().valid('true', 'false')).optional(),
